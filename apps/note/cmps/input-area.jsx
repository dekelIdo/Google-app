export class InputArea extends React.Component {
    ...
    onToggleVideo = () => {
        ...
        this.setState({ isVideoOpen: isVideoOpen ? false : true })
    }
   

    render() {
        const { text, title, backgroundColor } = this.props.state.note
        const { onAddNote, toggleFocus, handleChange, onChangeColor, onImageChange, onInputVideo ,onTypeUrlVideo} = this.props
        const classColor = 'colors'
        const { isColorShown, isVideoOpen, videoUrl } = this.state
        return <form className="note-onfocus" style={{ backgroundColor: backgroundColor, transition: '0.5s' }} onSubmit={this.getOptions}>
            ...
            <div className="container-input-btn">
                ...
                <span onClick={this.onToggleVideo} name="video" className="fa video"></span>
