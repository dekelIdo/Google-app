...
function getBooks() {
    let books = _loadFromStorage() || gBooks
    return books
}
...
function addReview(bookId, review) {
    ...
    const reviewNew = _creatReview(review)
    if (book.review) book.review.unshift(reviewNew)
    else book.review = [reviewNew]
    _saveBooksToStorage(books)
    // return Promise.resolve(book)
}
function _creatReview(review) {
    const currReview = { ...review, id: utilService.makeId() }
    return currReview
}
function getPrevPageIdx(boodId){
    let books = _loadFromStorage() || gBooks
    const bookIdx = books.findIndex(book => book.id === boodId)
    const prevIdxBook = bookIdx - 1 < 0 ? 0 : bookIdx - 1
    return books[prevIdxBook].id
}
function getNextPageIdx(boodId) {
    let books = _loadFromStorage() || gBooks
    const bookIdx = books.findIndex(book => book.id === boodId)
    const nextIdxBook = bookIdx + 1 === books.length ? 0 : bookIdx + 1
    return books[nextIdxBook].id
}
...
function getGoogleBook(text) {
    return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${text}`).then(res => res.data.items)

}
function addBook(book) {
    let books = _loadFromStorage() || gBooks
    const bookCheak = books.find(oldBook => oldBook.id === book.id)
    if (bookCheak) return Promise.reject()
    const bookAdd = _createBook(book)
    books.unshift(bookAdd)
    _saveBooksToStorage(books)
...
function _createBook(book) {
    return {
        ...
        thumbnail: book.volumeInfo.imageLinks.thumbnail ? book.volumeInfo.imageLinks.thumbnail : 'no image',
...
function findBookById(id) {
    ...
    const books = _loadFromStorage() || gBooks
